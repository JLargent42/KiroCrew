...
_configured_live_port="${KIROCREW_POD_LIVE_PORT-}"
if [ -z "${KIROCREW_POD_LIVE_PORT+x}" ]; then
  _configured_live_port="$DEFAULT_LIVE_PORT"
fi
_live_port_raw="$_configured_live_port"
case "$_configured_live_port" in
  ''|*[!0-9]*) _configured_live_port="" ;;
esac
while [ -n "$_configured_live_port" ] && [ "${_configured_live_port#0}" != "$_configured_live_port" ]; do
  _configured_live_port="${_configured_live_port#0}"
done
[ -n "$_configured_live_port" ] || _configured_live_port=0
if [ "${#_configured_live_port}" -le 5 ]; then
  CONFIGURED_LIVE_PORT=$((10#$_configured_live_port))
else
  CONFIGURED_LIVE_PORT=0
fi
if [ "$CONFIGURED_LIVE_PORT" -lt 1 ] || [ "$CONFIGURED_LIVE_PORT" -gt 65535 ]; then
  echo "pod-e2e: ignoring KIROCREW_POD_LIVE_PORT='$_live_port_raw' (want a decimal port in 1..65535); using $DEFAULT_LIVE_PORT" >&2
  CONFIGURED_LIVE_PORT="$DEFAULT_LIVE_PORT"
fi
