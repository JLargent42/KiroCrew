...
class TaskRunner:
    ...
    async def adopt_task_rows(self) -> "_runner_adapter.AdoptReport | None":
        ...
        its row is settled ``unknown_side_effect``. One sweep at a time: a
        call that overlaps the sweep ``attach_task_admission`` started joins
        it instead of adopting the same rows twice.
        """
    ...
    def _taskq_begin_run(self, run: Project) -> None:
    ...
    def _taskq_end_run(self, run: Project) -> None:
        ...
        if run.status == "completed":
            ...
            handle.cancel(f"run {run.status}")
    ...
    async def _execute_single_task(
        ...
        # else. A cancel that lands while it waits ends it here.
        try:
        ...
        if handle is not None:
            handle.running({"index": task.index, "title": task.title[:120]})
        ...
        try:
            ...
            if handle is not None:
                if success:
                    ...
                    handle.cancel(task.error or "run paused")
                else:
                    handle.fail(task.error or "step failed")
        if service is not None and run.workflow_run_id:
            try:
                await service.step(
                    run.workflow_run_id,
