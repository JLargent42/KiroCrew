...
def staged_template(posture: str) -> Iterator[Path]:
    ...
    A posture launch deploys the bundled file. A ``none`` launch deploys the base
    variant, which has to exist on disk for ``aws cloudformation deploy``; where
    it exists is the security question. Whatever ``run_aws`` reads executes with
    the launcher's credentials, so a template at a fixed path an in-sandbox
    agent could write would let a prompt-injected agent hand CloudFormation an
    arbitrary stack. The staging therefore lives under ``<config_dir>/run/`` --
    the runtime exec dir the agent sandbox denies read AND write on (see
    ``security/paths.py``; the sandbox launcher scripts and run markers live
    there for the same reason) -- in a fresh 0700 directory whose name is not
    predictable, written with ``O_NOFOLLOW|O_EXCL`` so a pre-planted link cannot
    redirect it, and removed once the deployment child has exited. Nothing
    outside the child ever sees the path.
    """
    if posture in ("workload", "login"):
        yield _template_path()
        return
    ...
    path = stage / "kirocrew-ec2-base.yaml"
    try:
        flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0)
        fd = os.open(path, flags, 0o600)
        with os.fdopen(fd, "w", encoding="utf-8") as handle:
            handle.write(base_template_text())
        yield path
    finally:
...
def template_path_for_posture(posture: str) -> Path:
    """The ``--template-file`` a DRY RUN shows for *posture*. The bundled file for
    a posture launch; for ``none`` a placeholder naming the base variant, since
    the real path exists only inside :func:`staged_template` for one deploy."""
    if posture in ("workload", "login"):
        return _template_path()
    return Path("<staged kirocrew-ec2-base.yaml>")
...
def build_deploy_argv(
    *,
    ...
    ``template_file`` is the path the deploy hands to the child -- for a ``none``
    launch the base variant :func:`staged_template` yields for exactly one deploy.
