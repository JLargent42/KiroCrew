...
``managed == "true"`` and the launch tag lives under its own key. ``MANAGED_TAG_KEY``
is IMPORTED here, never re-spelled: a second copy of a value whose meaning is
enforced by IAM conditions elsewhere is a copy that can drift out of agreement
with the policy that gates it.

**The launch tag's key is not spelled here either.** The module that writes the
tag onto a task owns that key, and this module cannot import it, so
:func:`classify_task` and :func:`plan_teardown` take it from the caller as a
required keyword (``launch_tag_key``). A private spelling here would be a second
copy that nothing checks against the writer: the moment the two disagree, every
task classifies as another launch's, teardown deletes nothing, and it reports
success while the task keeps billing. Refusing to hold the value is what makes
...
#: Registered as a distinct provisioner rather than reusing ``aws_ec2``: the
#: built-in id gates ``size_key`` validation against the EC2 instance ladder in
#: ``launch_job.py:309-310``, and a Fargate cpu/memory pair is not on that ladder.
FARGATE_PROVISIONER_ID = "aws_fargate"

#: The only value ``MANAGED_TAG_KEY`` may hold. Spelled here because this module
#: COMPARES against it; the key itself is imported from its owner.
...
def classify_task(
    sighting: TaskSighting, *, launch_tag: str, launch_tag_key: str, started_by: str
) -> Ownership:
    """Decide what *sighting* is with respect to *launch_tag*.

    *launch_tag_key* is the tag key carrying launch identity, and the caller
    supplies it because its owner is the module that writes the tag onto the task.
    This module refuses to keep a second spelling of it: a key spelled here that
    the writer does not use matches no real task, and then every task classifies
    as another launch's and teardown deletes nothing while reporting success.

    *started_by* is the ``startedBy`` value this launcher stamps on the tasks it
    runs. It decides between the two marked-but-not-ours outcomes: a marked task
    whose tag names another launch is :attr:`Ownership.OTHER_LAUNCH` only when
    someone else started it, and :attr:`Ownership.MISLABELLED` when this launcher
    did. Without that split the classifier calls a task this launcher started
    "another launch's", and teardown skips it in silence while it bills.
...
def plan_teardown(
    ...
    *launch_tag_key* is the tag key carrying launch identity. The caller supplies
    it because its owner is the module that writes the tag, and this module
    refuses to keep a second spelling of it (see :func:`classify_task`).
    *started_by* is the ``startedBy`` value this launcher stamps on the tasks it
    runs, and it is required because the ambiguous case below is defined by it:
    without it there is no way to tell a task this launcher plausibly started from
    a foreign one, so the refusal that case exists for could not fire.
    ...
    for sighting in sightings:
        kind = classify_task(
            sighting, launch_tag=launch_tag, launch_tag_key=launch_tag_key, started_by=started_by
        )
    ...
    if mislabelled:
        parts.append(
            f"{len(mislabelled)} running task(s) were started by this launcher "
...
class FargateSigninHandle:
    ...
    RFC §11 treats interactive sign-in as the hard part of running the crew on
    Fargate. That is wrong about this container, and the evidence is in the image
    rather than in an argument: ``runtime/Dockerfile:130`` states the credential is
    "Supplied at run time, never baked in", ``supervisor/__main__.py:422`` calls
    ``require_api_key(env)`` before serving, and a search of the whole runtime
    subtree for device-code, SSO, OAuth or interactive-login strings returns zero
    hits. The container is handed a key and refuses to boot without one; nobody
    ever signs it in.
