...
class AgentConfig:
    ...
    recovery_backoff_base_secs: float = field(
        ...
        metadata=_meta(
            ...
            "ACP runtime, task re-dispatch); each further attempt doubles it with "
            "jitter. One schedule for every layer, so layers never retry in "
            "lock-step. The gateway-daemon supervisor keeps its own pinned floor. "
            "Clamped to 0.1..60.",
            restart=True,
        ),
    )
    recovery_backoff_max_secs: float = field(
        ...
        metadata=_meta(
            ...
            "Longest delay between two recovery attempts on the shared ladder; a "
            "server-stated retry hint is honoured up to this cap. Clamped to "
            "1..3600 and never below the base.",
            restart=True,
        ),
    )
