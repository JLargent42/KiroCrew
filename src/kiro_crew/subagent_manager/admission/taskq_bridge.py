...
class _TaskqBridgeMixin(ManagerComponent):
    ...
    def taskq_open(self, cfg: Any, *, home: Any) -> "_taskq.TaskStore | None":
        ...
        The manager's startup worker attaches a successful result on the loop.
        """
        ...
        try:
            ...
            _glue_logger.error("%s; spawns are refused until it opens", reason)
            return None
    ...
    def taskq_settle(self, info: SubagentInfo) -> None:
        ...
        def _propagate() -> None:
            ...
            # is consistent. Touches live-run state, so it runs on the loop.
        ...
        if loop is None or not type(self).pump_off_loop:
            try:
                store.finish(info.id, state, **finish_kw)
            except _taskq.TaskStoreUnavailable:
        ...
        async def _settle() -> None:
            ...
            # other posted write), the propagation back on the loop after it.
            try:
                await store.run(store.finish, info.id, state, **finish_kw)
            except _taskq.TaskStoreUnavailable:
                _glue_logger.debug("taskq: settle of %s failed", info.id, exc_info=True)
                return
            except Exception:
