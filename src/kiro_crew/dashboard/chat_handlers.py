    ...
    for name, slot in list(state._slots.items()):
        ...
        async with contextlib.AsyncExitStack() as _stack:
            ...
            if not is_dashboard_user and session_key != _history_key_for(name):
                ...
                continue
            if slot.is_remote:
                # A bound slot's model lives on the PEER. Everything below is
                # local machinery — reset the local session, write slot.model —
                # and none of it reaches the machine that answers this slot's
                # turns, so applying it would leave the header advertising a
                # model the peer never received: the same divergence the single
                # -slot handler forwards to avoid. Forwarding is deliberately
                # NOT done here either: "set the model for all of MY chat
                # slots" must not reach across the network and rewrite live
                # conversations on other machines. The single-slot picker is how
                # a bound session's model is changed, and it forwards.
                # Classified before the equality check so a bound slot is
                # reported as out of scope rather than as "already on it".
