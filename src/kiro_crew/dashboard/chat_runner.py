    ...
    try:
        ...
        if _retry_eligible:
            ...
            try:
                ...
                logger.debug(
                    "model access fallback: set_model(%r) failed; surfacing " "entitlement error",
            ...
            logger.warning(
                "model access fallback: slot %s model %r not entitled; " "re-prompting on %r",
                slot.key,
                _rejected_id,
                _access_fb_candidate,
            )
            ...
            #
            # The set_model above is a provider RPC that yields the event loop,
            # so a Stop (or a user follow-up) can land between the elif's entry
            # guard and here. Re-check the same live-stop signals every sibling
            # requeue site checks right before enqueue: a message the user has
            # since stopped or replaced must not replay ahead of it. The model is
            # already swapped and the notice already shown, which is harmless; we
            # simply abandon the replay and let the turn end.
            if (
                ...
                # The replay is the user's ORIGINAL message, so the reset at turn
                # start cannot tell it from a fresh turn; this latch tells it to
                # preserve the one-shot flag for that replay alone.
