...
def _grep_hit(path: str, line: int, preview: str, label: str = "") -> dict:
    ...
    if label:
        # The label is document CONTENT, not a shape this endpoint chose: a
        # workbook's is its sheet title. It reaches the response and the audit
        # resource string, so it takes the same two passes the preview does --
        # redaction first, then the cut, or the cut would leave an unmatchable
