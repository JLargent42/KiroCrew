...
class TurnDriver:
    ...
    async def _steer_deny_cause(self, event: Any) -> None:
        ...
        :func:`kiro_crew.dashboard.state.steer_refusal_notice`; a failure there
        ...
        while this coroutine unwinds, then the cancellation re-raises.

        The import is function-local: ``dashboard.state`` is the heavy end of
        the process and this module is the channel-neutral core.
        """
        cause = getattr(self.decider, "last_deny_cause", "") if self.decider is not None else ""
        if cause != DENY_CAUSE_APPROVAL_TIMEOUT:
            return
        from kiro_crew.dashboard.state import steer_refusal_notice
