...
# The reprieve is bounded: a daemon whose self-report keeps saying "serving"
# while it never answers a single ping is not overloaded, it is wedged with a
# live writer thread. After this many CONSECUTIVE averted kills (each one
# ``_LIVENESS_MAX_CONSECUTIVE_FAILURES`` missed pings, so 4 × 3 × 30 s = six
# minutes without one answered ping) the kill path runs regardless of the
# report. One answered ping ends the run and starts the count over.
_LIVENESS_MAX_DEFERRALS = 4
...
class GatewayManager:
    ...
    async def _liveness_probe_loop(self) -> str:
        ...
        :meth:`_alive_but_overloaded`), or once the report has vouched for it
        ``_LIVENESS_MAX_DEFERRALS`` times in a row with no ping answered in
        between. Never returns normally — either the
        ...
        deferrals = 0  # averted kills since the last answered ping
        while True:
            await asyncio.sleep(_LIVENESS_PING_INTERVAL_SECS)
            if self._stopping:
            ...
            if report is not None and deferrals >= _LIVENESS_MAX_DEFERRALS:
                unanswered = deferrals * _LIVENESS_MAX_CONSECUTIVE_FAILURES + consecutive_failures
                logger.error(
                    "mcp-gateway: the daemon's self-report says pid=%s is serving, "
                    "but %d consecutive averted kills (%d pings, %ds) went by without "
                    "one answered ping -- treating the report as a wedged writer, not "
                    "life; killing",
                    report.get("pid"),
                    deferrals,
                    unanswered,
                    int(unanswered * _LIVENESS_PING_INTERVAL_SECS),
                )
                return (
                    f"zombie detected: self-report vouched {deferrals} times in a row "
                    f"but {unanswered} consecutive pings went unanswered over "
                    f"{int(unanswered * _LIVENESS_PING_INTERVAL_SECS)}s"
                )
