...
def agent_spec_path(name: str, agents_dir: Path | None = None) -> Path | None:
    """Return the kiro spec file for *name* in *agents_dir* (default: the
    user-level agents dir), or ``None`` if absent.
    ...
    *name* is validated against the shared agent-name grammar BEFORE it reaches
    ...
    if not _AGENT_NAME_RE.match(name or ""):
        return None
    agents_dir = kiro_agents_dir_path() if agents_dir is None else Path(agents_dir)
    if not agents_dir.is_dir():
        return None
