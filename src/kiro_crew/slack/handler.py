    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    try:
        ...
        try:
            if use_slack_stream and stream_ts:
                ...
                # it arrives, and a refused append is treated as recoverable
                # delivery-debt (a designed follow-up), NOT a turn failure: the
                # model produced a complete answer and the drop is transient. So
                # a stream that was used counts as answer-reached. ``_stream_
                # delivered`` records whether ANY append was confirmed, kept for
                # the no-stream contrast and future delivery-debt accounting.
    ...
    try:
        ...
        try:
            ...
            if options:
                try:
                    _fallback, _ = redact_exfiltration_urls(
                        "*Options:*\n" + "\n".join(f"• {o}" for o in options)
                    )
                    _fallback, _ = redact_credentials(_fallback)
