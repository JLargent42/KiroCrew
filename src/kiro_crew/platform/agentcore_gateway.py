...
# kiro-cli ``session/new`` deserializes an untagged ``McpServer``. HTTP
# requires ``type`` plus a ``headers`` array (empty is fine). Retract is the
# absence of an entry, never a placeholder element.
ACP_HTTP_TYPE = "http"
...
def _identity_on(session_key: str = "", *, agent: str = "") -> bool:
    ...
    if not permitted:
        return False
    return _current_posture() is not None
...
def _current_posture() -> str | None:
    """The EFFECTIVE posture: the ceiling when one is loaded, else the launch
    env / home authoring (``agentcore_aws.resolved_posture``). A CloudFormation
    launch configures the posture through the instance unit's environment and
    may run with no policy document at all; reading only
    ``current_context().governance`` there answers ``None`` and the documented
    launch path never receives its Gateway. A present ceiling still outranks
    every fallback inside the resolver, so a policy that disabled AgentCore
    cannot be undone by leftover env."""
...
def _sidecar_fingerprint(session_key: str) -> str:
    """Sidecar state plus bytes, or absent. Detects attach mutations."""
    path = inbound_sidecar_path(session_key)
    try:
...
def session_gateway_servers(session_key: str, *, agent: str = "") -> list[dict[str, Any]]:
    ...
    if data.get("denied") is True:
        # Retract is the ABSENCE of a session entry. The Gateway is only ever
        # session-injected -- rebuild never writes it into the agent file and
        # pops a persisted managed leftover (``_is_managed_gateway_leftover``)
        # -- so there is no same-named ``--agent`` entry to outrank, and a
        # placeholder element would rest on how kiro-cli treats a disabled
        # wire entry, which nothing here verifies.
        return []
    url = data.get("url")
...
def _login_gateway_spec() -> dict[str, Any] | None:
    """Gateway URL for a login sidecar — a real Gateway host, never the SigV4 proxy.

    ``gateway_mcp_spec()`` rewrites to ``127.0.0.1`` when env posture is
    still ``workload`` (CFN leftover / pre-restart). kiro-cli cannot run
    an OAuth challenge against that listener. Prefer the configured URL;
    fall back to an adapter spec. Either must be an AgentCore Gateway
    hostname: the sidecar carries the human's bearer, and any other https
    destination would receive it — the same rule the workload proxy applies
    to its SigV4 upstream.
    """
    ...
    if is_agentcore_gateway_url(real):
        return sanitize_gateway_spec({"url": real})
    sanitized = _gateway_spec_from_adapter()
    url = str((sanitized or {}).get("url") or "")
    if is_agentcore_gateway_url(url):
        return sanitized
    return None
