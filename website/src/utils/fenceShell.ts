 ...
 *
 * The old 6 s value raced healthy-but-slow shells: a bash login shell only
 * reports ready at its first prompt, i.e. after the full profile chain, which
 * on heavyweight dev hosts routinely takes longer than 6 s (#10822). The
 * deadline is a shared constant so the button's own fallback below can never
 * declare failure before the dispatcher has ruled.
 */
export const RUN_IN_TERMINAL_READY_DEADLINE_MS = 15_000
