...
export function kiroPrerequisiteIsBlocking(
  ...
  if (isEmbeddedPane() && status.installed && !status.authenticated) return true
...
function EmbeddedSigninPending({
  ...
  const handleOpenSettings = () => {
    ...
    // lands on Remote Instances settings without leaving the pane context.
    try {
      window.parent.postMessage({ type: 'mc-navigate', path: '/settings/instances' }, '*')
    } catch {
