...
- Status: accepted (owner decision 2026-09-12 14:21: ONE PR, no design questions
  ...
  longer kills a daemon whose 10s liveness ping says alive; the stub reconnect
  budget rises from 60s to 600s) is already on the branch and is treated as
  landed. §12 names the internal waves; §13 records every decision with the
...
|---|---|---|---|---|---|
| 1 (slice 0, on branch) | — | supervisor self-report gate + 10s liveness ping (`mcp_gateway/self_report.py`), stub reconnect budget 600s | small | — | mcp-gateway-daemon-lifecycle |
