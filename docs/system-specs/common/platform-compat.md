...
| FD soft limit | `raise_nofile_soft_limit(n)` to raise; `nofile_soft_limit()` to read (0 = unavailable/unbounded, including Windows) | direct `resource.setrlimit` / `getrlimit` |
