...
- **An unexpected spawned-process exit is restarted, but never blindly.** After
  ...
  governance and admission: an affirmative policy denial ends supervision, while an
  evaluation error refuses only that attempt, is audited as an error, and keeps the
  current fast or steady cadence so the next attempt re-vets. Boot and interactive spawn
  preserve main's best-effort disposition on an evaluator error: a transient verdict is
  no opinion, so the app starts with a warning while affirmative policy denials still
  refuse activation. The restart loop alone refuses transient verdicts per attempt
  because it has a retry cadence; boot and interactive spawn have no such retry surface.
  Every deliberate stop and every
