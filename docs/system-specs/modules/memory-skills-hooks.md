...
`503 store_unavailable` and preserves the current document. The dashboard's
store-unavailable response names the store and gives fixed configuration/database
access guidance; internal exception text, including workspace paths, is never
inspected or used as public response content. Startup-recovery exceptions receive
