...
class AgentConfig:
    ...
    lane_weights: dict[str, int] = {}    # per-lane overrides keyed by root session key or 'system'; unlisted lanes weigh 1. Each value load-time clamped to [1, 64]; non-string keys dropped
    child_reserve: int = 1               # execution slots depth-0 tasks may never take while a nested task is queued or a parent waits on children; 0 disables. Load-time clamped to [0, 8]
    recovery_backoff_base_secs: float = 2.0    # first retry delay of the shared recovery ladder (tool call / backend / ACP runtime / task re-dispatch); doubles with equal jitter. The gatewayd supervisor's rung is pinned and does not follow it. Load-time clamped to [0.1, 60]; restart=True. See modules/session.md § Recovery ladder
    recovery_backoff_max_secs: float = 120.0   # cap on that delay; a server retry hint is honoured up to it. Load-time clamped to [1, 3600], never below the base; restart=True
    ...
    lane_weights: dict[str, int] = {}          # weights keyed by root session key or "system"; values clamped to [1, 64], default 1. Live
    child_reserve: int = 1                     # slots a depth-0 start may never take while a nested row or a resume waits; also lifts an adaptive squeeze to adaptive_floor + child_reserve while a parent waits (never above max_subagents). 0 disables. Load-time clamped to [0, 8]. Live. See modules/subagent.md § Fairness lanes and the child reserve
