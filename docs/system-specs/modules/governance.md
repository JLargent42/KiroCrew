...
raises, so a leftover JWT cannot stay usable). The drain runs under
the session registry's closing gate, entered under the registry lock
before the snapshot, so an allocation racing the save is refused
(`SessionClosingError`) rather than landing behind it; allocations
already past the gate are given a settle budget to register and be
drained. A reservation still in flight when that budget runs out is
never waited past: the revoke raises, the drained providers are
force-killed, and the gate STAYS closed until the gateway restarts --
reopening it would admit a provider spawned with the pre-save identity.
The identity PUT reports that as 503 `restart_required`. Off, Login, and Workload all
