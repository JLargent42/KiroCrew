...
A raise pumps the queue through the same staggered drain a config raise uses.
...
- **Store.** `SubagentManager.__init__` opens `$KIROCREW_HOME/tasks/tasks.db`
  ...
  attachment without cancelling the worker. Synchronous constructors without a
  ...
  cause in `error`) -- accepted work must never live only in memory.
...
- **Wakes.** Only `resume_grant` writes `running` (`wake_wait(to=running)`,
  slot granted, runtime resident). A live parent whose last child ended stays
...
- **`_queue` is a bounded window**, never the whole backlog. It holds at most
  ...
  `has_pending_work_for` / `batch_members_pending` / the stuck-wave sweep /
  `cancel_for_parent` all add the store-only rows (`taskq_overflow`,
  `taskq_batch_pending`, `taskq_pending_ids_for`).
...
- **Resume hold (`wait_resume_granted`, `GET /api/spawn/{id}/resume`).**
  ...
  `GET /api/spawn/lanes` → `admission.lane_snapshot()`.
