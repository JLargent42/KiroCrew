...
- **Flow**: Download to temp file → sniff the raster type → promote into the dashboard's uploads directory (`kiro_crew.uploads.create_upload_file`: `<data_home>/uploads/<uuid hex>_<sanitized name>.<sniffed ext>`, `O_EXCL`, `0o600`, all-or-nothing write) → `append_attachment_context()` writes the row as `![image](/abs/uploads/…)` — the dashboard composer's own shape, so the transcript renders the picture after the turn — and `_send_prompt()` finds the path inside that markup, base64-encodes it, and sends it as a `{"type": "image"}` content block to kiro-cli. Promotion is skipped (`events._inbound_images_persist`) for a thread whose conversation persists nothing — a durable `temporary`/`incognito` flag, a `!incognito`/`!temporary` token in the message itself, or a linked dashboard session in a restricted mode — and the image then stays a temp file on the lifecycle below.
