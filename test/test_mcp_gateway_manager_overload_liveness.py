...
class TestOverloadedDaemonIsKept:
    ...
    @pytest.mark.asyncio
    async def test_the_reprieve_is_capped_at_max_deferrals(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        clock: None,
        ...
        """A daemon that keeps writing ``is_serving: true`` but never answers a
        ping is wedged with a live writer, not overloaded: after
        ``_LIVENESS_MAX_DEFERRALS`` consecutive averted kills the next
        threshold kills it, report or no report."""
        manager = _manager(tmp_path)
        _write_jsonl(report_path, _probe())
        per_threshold = mgr._LIVENESS_MAX_CONSECUTIVE_FAILURES
        cap = mgr._LIVENESS_MAX_DEFERRALS
        assert cap == 4

        # One threshold short of the cap being exceeded: still deferring.
        verdict, _ = await _run_probe_loop_until(
            manager, monkeypatch, ping_results=[False] * (per_threshold * cap)
        )
        assert verdict is None
        assert len(counters) == cap

        # The (cap + 1)-th threshold with the same fresh report: killed.
        counters.clear()
        manager = _manager(tmp_path)  # the harness stopped the first one
        verdict, _ = await _run_probe_loop_until(
            manager, monkeypatch, ping_results=[False] * (per_threshold * (cap + 1))
        )
        assert verdict is not None
        assert verdict.startswith("zombie detected: self-report vouched 4 times in a row")
        assert f"{per_threshold * (cap + 1)} consecutive pings went unanswered" in verdict
        assert len(counters) == cap, "the refused deferral is not counted as an averted kill"

    @pytest.mark.asyncio
    async def test_one_answered_ping_resets_the_deferral_count(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        clock: None,
        ...
        per_threshold = mgr._LIVENESS_MAX_CONSECUTIVE_FAILURES
        cap = mgr._LIVENESS_MAX_DEFERRALS
        # cap deferrals, ONE answered ping, cap deferrals again: 2 × cap averted
        # kills and no verdict, where the same misses without the answer kill.
        misses = [False] * (per_threshold * cap)
        verdict, _ = await _run_probe_loop_until(
            manager, monkeypatch, ping_results=misses + [True] + misses
        )
        assert verdict is None
        assert len(counters) == 2 * cap
