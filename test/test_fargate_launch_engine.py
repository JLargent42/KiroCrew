...
from kiro_crew.cloud import fargate_engine
...
def _plan(sightings: list[TaskSighting]) -> fargate_engine.TeardownPlan:
    return plan_teardown(
        sightings, launch_tag=TAG, launch_tag_key=LAUNCH_TAG_KEY, started_by=STARTED_BY
    )


def _classify(sighting: TaskSighting, *, launch_tag_key: str = LAUNCH_TAG_KEY) -> Ownership:
    return classify_task(
        sighting, launch_tag=TAG, launch_tag_key=launch_tag_key, started_by=STARTED_BY
    )
...
def test_provisioner_id_is_not_the_builtin_one() -> None:
    """A distinct id, because the built-in id gates EC2 size validation.

    ``launch_job.py`` validates ``size_key`` against ``sizes.get_tier`` only for
    ``BUILTIN_PROVISIONER_ID``. Reusing that id would refuse every Fargate
    cpu/memory pair before the engine ever saw it.
    """
    from kiro_crew.platform.interfaces import BUILTIN_PROVISIONER_ID

    assert FARGATE_PROVISIONER_ID != BUILTIN_PROVISIONER_ID
...
def test_classification_requires_a_launch_tag() -> None:
    with pytest.raises(ValueError, match="launch tag is required"):
        classify_task(
            _marked(), launch_tag="", launch_tag_key=LAUNCH_TAG_KEY, started_by=STARTED_BY
        )
...
def test_ownership_is_read_under_the_key_the_caller_supplies() -> None:
    """Identity is looked up under ``launch_tag_key`` and under nothing else.

    A task tagged under the caller's key is ours. The same task read under a key
    the writer does not use is NOT ours -- and a module that kept a private
    spelling of the key would be reading under exactly such a key whenever the
    writer's spelling differs. Because the task still carries this launcher's
    ``startedBy``, the misread does not pass as another launch's: it classifies
    MISLABELLED, and the plan declines to confirm instead of stopping nothing
    while reporting success.
    """
    sighting = _marked()
    assert _classify(sighting) is Ownership.OURS
    assert (
        _classify(sighting, launch_tag_key="kirocrew:not-what-the-writer-uses")
        is Ownership.MISLABELLED
    )
    plan = plan_teardown(
        [sighting],
        launch_tag=TAG,
        launch_tag_key="kirocrew:not-what-the-writer-uses",
        started_by=STARTED_BY,
    )
    assert plan.delete == (), "a key nobody writes must match no task"
    assert plan.confirmed is False, "a key nobody writes must not pass as a clean teardown"
    assert ARN in plan.warning


def test_identity_key_is_a_required_keyword_on_both_readers() -> None:
    """Neither reader may fall back to a default; the caller must name the key."""
    for fn in (classify_task, plan_teardown):
        param = inspect.signature(fn).parameters["launch_tag_key"]
    ...
    with pytest.raises(TypeError, match="launch_tag_key"):
        classify_task(_marked(), launch_tag=TAG, started_by=STARTED_BY)  # type: ignore[call-arg]
    with pytest.raises(TypeError, match="launch_tag_key"):
        plan_teardown([], launch_tag=TAG, started_by=STARTED_BY)  # type: ignore[call-arg]


def test_module_spells_no_tag_key_of_its_own() -> None:
    """The module holds no ``kirocrew:`` tag key literal and exports none.

    The one key it compares against, ``MANAGED_TAG_KEY``, is imported from its
    owner. Any ``"kirocrew:..."`` literal here would be a second copy of a value
    another module owns, unchecked against that owner, and free to drift.
    """
    ...
    exported_keys = [
        name
        for name in fargate_engine.__all__
        if name.endswith("_KEY") and name != "MANAGED_TAG_KEY"
    ]
    assert exported_keys == [], exported_keys
...
def test_started_by_is_required_so_the_refusal_cannot_be_switched_off() -> None:
    ...
    with pytest.raises(TypeError, match="started_by"):
        plan_teardown([sighting], launch_tag=TAG, launch_tag_key=LAUNCH_TAG_KEY)  # type: ignore[call-arg]
    with pytest.raises(TypeError, match="started_by"):
        classify_task(_marked(), launch_tag=TAG, launch_tag_key=LAUNCH_TAG_KEY)  # type: ignore[call-arg]
