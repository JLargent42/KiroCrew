...
class TestRestartExitedBackend:
    ...
    def test_spawn_chokepoint_rechecks_activation_policy(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(bmod, "get_app_manifest", lambda _name: _manifest("server.py"))
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            bmod,
            "_start_app_backend_body",
            lambda *_args: pytest.fail("denied app reached the spawn primitive"),
        )
        assert bmod._start_app_backend("app") is None
        assert "app" not in bmod._processes

    def test_spawn_chokepoint_transient_verdict_does_not_block_spawn(
        self,
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
    ) -> None:
        ...
        def _start(app_name: str, _manifest: Any) -> AppProcess:
            with bmod._lock:
                bmod._processes[app_name] = replacement
            return replacement

        monkeypatch.setattr("kiro_crew.platform.governance_profiles.governance_permits", _raise)
        monkeypatch.setattr(bmod, "get_app_manifest", lambda _name: _manifest("server.py"))
        monkeypatch.setattr(bmod, "_start_app_backend_body", _start)

        with caplog.at_level(logging.WARNING):
            assert bmod._start_app_backend("app") is replacement
        assert bmod._processes["app"] is replacement
        assert sum("best-effort activation posture" in r.message for r in caplog.records) == 1
...
class TestBootSpawnGating:
    ...
    def test_governance_evaluation_error_still_boots(
        self,
        boot_env: dict[str, list[Any]],
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
    ) -> None:
        def _raise(_name: str, **kwargs: Any) -> Any:
            assert kwargs.get("fail_closed") is True
            raise RuntimeError("profile evaluator unavailable")

        monkeypatch.setattr(bmod, "_app_activation_denied", _raise)
        monkeypatch.setattr(bmod, "list_apps", lambda: [_app("unverifiable")])

        with caplog.at_level(logging.WARNING):
            assert bmod.start_enabled_app_backends() == []
        assert boot_env["dereg_agents"] == []
        assert boot_env["register"] == ["unverifiable"]
        assert boot_env["started"] == ["unverifiable"]
        assert sum("best-effort activation posture" in r.message for r in caplog.records) == 1
    ...
    def test_an_admission_revet_error_does_not_block_boot(
        self,
        boot_env: dict[str, list[Any]],
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
    ) -> None:
        """A transient admission evaluator failure preserves best-effort boot."""
        ...
        with caplog.at_level(logging.WARNING):
            assert bmod.start_enabled_app_backends() == []
        assert sum("best-effort activation posture" in r.message for r in caplog.records) == 1
        assert boot_env["register"] == ["shady"]
        assert boot_env["started"] == ["shady"]
