...
def test_none_launch_stages_the_base_variant_only_for_the_deploy_child(
    ...
    # A posture launch ships the bundled file itself.
    with ec2.staged_template("workload") as path:
        assert path.name == "kirocrew-ec2.yaml"
        assert "AWS::BedrockAgentCore::WorkloadIdentity" in path.read_text(encoding="utf-8")
...
def test_deploy_argv_carries_the_staged_file_and_the_dry_run_a_placeholder(
    ...
    assert Path(argv[argv.index("--template-file") + 1]).name == "kirocrew-ec2.yaml"
