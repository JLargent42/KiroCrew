...
def _check_handle(
    ...
    if live_port is not None:
        preamble.append(f"export KIROCREW_POD_LIVE_PORT={live_port!r}")
...
def test_auth_uses_the_canonical_handle_and_drops_a_url_path(tmp_path):
    ...
    assert res.returncode == 0, res.stdout + res.stderr
    assert "PASS:auth" in res.stdout, res.stdout
...
def test_safety_refuses_every_live_plane_port(tmp_path, port, live_port):
    ...
    assert f"FAIL:SAFETY — pod resolved to production port {port}, aborting" in res.stdout
...
def test_safety_accepts_a_normal_pod_port(tmp_path):
    ...
    assert res.returncode == 0, res.stdout + res.stderr
    assert "FAIL:SAFETY" not in res.stdout
...
def test_handle_schema_matches_all_producer_key_sets():
    ...
    shell_env = re.search(
        r'^_configured_live_port="\$\{([A-Z0-9_]+)-\}"$', shell_source, re.MULTILINE
    )
...
def test_a_supplied_healthy_code_is_taken_and_no_pod_verb_runs(stub_cli, tmp_path, code):
    ...
    assert res.returncode == 0, res.stdout + res.stderr
    assert "HEALTHY=1" in res.stdout, res.stdout
    assert f"PASS:health — supplied by the caller (health={code})" in res.stdout
    assert not (
